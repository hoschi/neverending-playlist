...
- **`neverending_scheduler_service.py`** - Enthält pure Scheduler-Entscheidungslogik: täglicher 02:00-Run ohne stündliches Catch-up sowie Berechnung der Wartezeit bis 02:00.
...
- **`neverending_scheduler.py`** - Daily Background-Scheduler, der bis 02:00 Lokalzeit schläft und dann den Import ausführt. Nutzt `neverending_songs_runs` (Status `SUCCESS`), um einen zweiten Lauf in derselben Stunde zu vermeiden. Optionaler Startup-Debug-Override (`DEBUG_SYNC_AT_STARTUP`) löst beim App-Start einmalig sofort einen Importlauf aus.
